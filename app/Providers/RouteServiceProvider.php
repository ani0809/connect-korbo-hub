  ...
  /**
    ...
    protected function configureRateLimiting()
    {
        ...
        RateLimiter::for('api-auth', function (Request $request) {
            $identity = (string) ($request->input('email') ?: $request->input('email_or_phone') ?: '');
            $identity = mb_strtolower(trim($identity));
            $key = optional($request->user())->id ?: ($request->ip() . '|' . $identity);

            return [
                Limit::perMinute(40)->by($key),
                Limit::perMinute(12)->by($request->ip() . '|burst')
            ];
        });

        RateLimiter::for('api-auth-reset', function (Request $request) {
            $identity = (string) ($request->input('email_or_phone') ?: '');
            $identity = mb_strtolower(trim($identity));
            $key = $request->ip() . '|' . $identity;

            return [
                Limit::perMinute(15)->by($key),
                Limit::perMinute(8)->by($request->ip() . '|reset')
            ];
        });
    }
