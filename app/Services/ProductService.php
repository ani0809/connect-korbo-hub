...
class ProductService
{
    public function store(array $data)
    {
        ...
        $tags = array();
        $tagsPayload = $collection['tags'] ?? null;
        $decodedTags = array();
        if (is_array($tagsPayload)) {
            $rawTags = $tagsPayload[0] ?? null;
            if (is_string($rawTags) && $rawTags !== '') {
                $decoded = json_decode($rawTags);
                if (is_array($decoded)) {
                    $decodedTags = $decoded;
                }
            }
        } elseif (is_string($tagsPayload) && $tagsPayload !== '') {
            $decoded = json_decode($tagsPayload);
            if (is_array($decoded)) {
                $decodedTags = $decoded;
            }
        }
