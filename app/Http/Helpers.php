...
/**
 * Path segment for Laravel's UrlGenerator when the disk root is `public/` (local uploads).
 * - Normal (`php artisan serve`, most hosts pointing vhost at `public/`): URL is `/uploads/...`
 * - Legacy cPanel where site root is the project folder (not `public/`): URL is `/public/uploads/...`
 */
if (!function_exists('local_public_upload_asset_path')) {
    function local_public_upload_asset_path(string $path): string
    {
        ...
        if (filter_var(env('PUBLIC_UPLOAD_LEGACY_PREFIX', false), FILTER_VALIDATE_BOOLEAN)) {
            return 'public/' . $path;
        }
...
if (!function_exists('uploaded_asset')) {
    function uploaded_asset($id)
    {
        ...
        if (($asset = Upload::find($id)) != null) {
            ...
            $path = ltrim(str_replace('\\', '/', (string) $asset->file_name), '/');
            if ($path === '') {
                return static_asset('assets/img/placeholder.jpg');
            }

            // Local uploads live under `public/`; URL shape depends on web server document root.
            if (config('filesystems.default') == 'local') {
                return static_asset(local_public_upload_asset_path($path));
            }
...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        ...
        return static_asset(local_public_upload_asset_path($path), $secure);
    }
}
...
if (!function_exists('get_setting')) {
    ...
    {
        $forcedPurchaseCode = '0a6fa84f-2d44-4364-b057-7fec2c3edd38';
        $forceDisabledSettings = [
            'wallet_system',
            'pickup_point',
            'conversation_system',
            'product_query_activation',
            'digital_product_manage_by_seller',
        ...
        if (in_array($key, $forceDisabledSettings, true)) {
            return 0;
        }

        if ($key === 'guest_checkout_activation' || $key === 'guest_checkout_active') {
            return 1;
        }

        if ($key === 'purchase_code') {
            return $forcedPurchaseCode;
        }

        try {
            if (!\Illuminate\Support\Facades\Schema::hasTable('business_settings')) {
                if ($key === 'homepage_select') {
                    return 'thecore';
                }
                return $default;
            }
        ...
        if ($key === 'homepage_select') {
            $allowedHomepages = ['classic', 'metro', 'minima', 'megamart', 'reclassic', 'thecore'];
            $setting = BusinessSetting::where('type', 'homepage_select')->first();
            $value = $setting ? trim((string) $setting->value) : '';
            return in_array($value, $allowedHomepages, true) ? $value : 'thecore';
        }
...
if (!function_exists('thecore_homepage_layout_available_sections')) {
    function thecore_homepage_layout_available_sections()
    {
        return [
            'hero_slider' => translate('Hero Slider'),
            'featured_categories' => translate('Featured Categories'),
            'featured_products' => translate('Featured Products'),
            'best_selling_todays' => translate('Best Selling + Todays Deal'),
            'banner_1' => translate('Banner Level 1'),
            'auction_products' => translate('Auction Products'),
            'classified_products' => translate('Classified Products'),
            'preorder_products' => translate('Newest Preorder Products'),
            'banner_2' => translate('Banner Level 2'),
            'home_categories' => translate('Category-wise Products (4 Product Carousel)'),
            'newest_products' => translate('Newest Products'),
        ];
    }
}

if (!function_exists('thecore_homepage_section_availability')) {
    function thecore_homepage_section_availability()
    {
        return [
            'auction_products' => addon_is_activated('auction'),
            'classified_products' => get_setting('classified_product') == 1,
            'preorder_products' => addon_is_activated('preorder'),
            'home_categories' => true,
        ];
    }
}

if (!function_exists('expand_thecore_home_category_instances')) {
    function expand_thecore_home_category_instances(array $layoutItems)
    {
        $homeCategoryIndexes = [];
        foreach ($layoutItems as $index => $item) {
            if (($item['section_key'] ?? null) === 'home_categories') {
                $homeCategoryIndexes[] = $index;
            }
        }

        if (count($homeCategoryIndexes) !== 1) {
            return $layoutItems;
        }

        $singleIndex = $homeCategoryIndexes[0];
        $singleItem = $layoutItems[$singleIndex];
        $existingCategoryId = $singleItem['settings']['category_id'] ?? null;
        if (!empty($existingCategoryId)) {
            return $layoutItems;
        }

        $rawHomeCategories = get_setting('home_categories');
        $homeCategories = json_decode($rawHomeCategories, true);
        if (!is_array($homeCategories) || count($homeCategories) === 0) {
            return $layoutItems;
        }
        ...
        foreach ($homeCategories as $categoryId) {
            $expandedItems[] = [
                'id' => 'thc-' . substr(md5(uniqid('', true)), 0, 10),
                'section_key' => 'home_categories',
                'enabled' => (bool) ($singleItem['enabled'] ?? true),
                'settings' => ['category_id' => (int) $categoryId],
            ];
        }

        array_splice($layoutItems, $singleIndex, 1, $expandedItems);
        return $layoutItems;
    }
}

if (!function_exists('thecore_homepage_default_layout')) {
    function thecore_homepage_default_layout()
    {
        $defaultKeys = [
            'hero_slider',
            'featured_categories',
        ...
        return array_map(function ($sectionKey, $index) {
            return [
                'id' => 'thc-' . ($index + 1) . '-' . substr(md5(uniqid('', true)), 0, 8),
                'section_key' => $sectionKey,
                'enabled' => true,
                'settings' => [],
            ];
        }, $defaultKeys, array_keys($defaultKeys));
    }
}

if (!function_exists('normalize_thecore_homepage_layout')) {
    function normalize_thecore_homepage_layout($layoutItems)
    {
        if (!is_array($layoutItems)) {
            return thecore_homepage_default_layout();
        }

        $allowedSections = array_keys(thecore_homepage_layout_available_sections());
        $normalized = [];
        $seenIds = [];
        ...
        foreach ($layoutItems as $item) {
            ...
            $sectionKey = isset($item['section_key']) && in_array($item['section_key'], $allowedSections, true)
                ? $item['section_key']
                : null;
            ...
            $candidateId = isset($item['id']) && is_string($item['id']) ? trim($item['id']) : '';
            if ($candidateId === '' || isset($seenIds[$candidateId])) {
                $candidateId = 'thc-' . substr(md5(uniqid('', true)), 0, 10);
            }
        ...
        return count($normalized) > 0 ? $normalized : thecore_homepage_default_layout();
    }
}

if (!function_exists('get_thecore_homepage_layout')) {
    function get_thecore_homepage_layout()
    {
        try {
            if (!\Illuminate\Support\Facades\Schema::hasTable('business_settings')) {
                return thecore_homepage_default_layout();
        ...
        $rawLayout = $setting ? $setting->value : null;
        if (!$rawLayout) {
            return thecore_homepage_default_layout();
        }
        ...
        return expand_thecore_home_category_instances(normalize_thecore_homepage_layout($decoded));
    }
}
...
if (!function_exists('addon_is_activated')) {
    function addon_is_activated($identifier, $default = null)
    {
        ...
        if (in_array($identifier, $forceDisabledAddons, true)) {
            return false;
        }
...
if (!function_exists('get_system_language')) {
    function get_system_language()
    {
        ...
        return $language_query->first() ?: (object) [
            'code' => 'en',
            'name' => 'English',
            'rtl' => 0,
        ];
...
if (!function_exists('get_all_active_language')) {
    function get_all_active_language()
    {
        ...
        return $languages->count() ? $languages : collect([
            (object) [
                'code' => 'en',
                'name' => 'English',
                'rtl' => 0,
...
if (!function_exists('get_session_language')) {
    function get_session_language()
    {
        ...
        $language_query = Language::query();
        return $language_query->where('code', Session::get('locale', Config::get('app.locale')))->first() ?: (object) [
            'code' => 'en',
            'name' => 'English',
            'rtl' => 0,
        ];
