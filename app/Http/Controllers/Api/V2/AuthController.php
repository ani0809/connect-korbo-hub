...
{
    private function apiErrorResponse($message, $status = 400, $user = null, array $extra = [])
    {
        $payload = array_merge([
            'result' => false,
            'success' => false,
    ...
    public function signup(Request $request)
    {
        ...
        if ($validator->fails()) {
            \Log::warning('API signup validation failed', [
                'register_by' => $request->register_by,
                'has_email_or_phone' => !empty($request->email_or_phone),
                'ip' => $request->ip(),
                'errors' => $validator->errors()->all(),
            ]);
            return $this->apiErrorResponse($validator->errors()->all(), 422);
        }
    ...
    public function confirmCode(Request $request)
    {
        ...
        if ($validator->fails()) {
            \Log::warning('API confirm code validation failed', [
        ...
        if ($user->verification_code == $request->verification_code) {
            ...
            \Log::warning('API confirm code mismatch', [
                'user_id' => optional($user)->id,
                'ip' => $request->ip(),
            ]);
            return $this->apiErrorResponse(translate('Code does not match, you can request for resending the code'), 422);
        }
    }
    ...
    public function login(Request $request)
    {
        ...
        if (!$delivery_boy_condition && !$seller_condition) {
            if (\App\Utility\PayhereUtility::create_wallet_reference($request->identity_matrix) == false) {
                \Log::warning('API login identity matrix error', [
                    'login_by' => $request->login_by,
                    'email' => $request->email,
                    'ip' => $request->ip(),
                ]);
                return $this->apiErrorResponse('Identity matrix error', 401);
            }
        }
        ...
        if ($user != null) {
            if (!$user->banned) {
                if (Hash::check($request->password, $user->password)) {
                    if($user->user_type=='seller' && optional($user->shop)->registration_approval == 0){
                        \Log::warning('API login blocked: seller under review', [
                            'user_id' => $user->id,
                            'email' => $user->email,
                            'ip' => $request->ip(),
                        ]);
                        return $this->apiErrorResponse(translate('Your seller account is under review. We will notify you once approved.'), 401);
                    }else{
                        $tempUserId = $request->has('temp_user_id') ? $request->temp_user_id : null;
                        return $this->loginSuccess($user,'', $tempUserId);
                    }
                    ...
                    return $this->apiErrorResponse(translate('Unauthorized'), 401);
                }
            } else {
                \Log::warning('API login failed: user banned', [
                    'user_id' => optional($user)->id,
                    'email' => $request->email,
                    'ip' => $request->ip(),
                ]);
                return $this->apiErrorResponse(translate('User is banned'), 401);
            }
        } else {
            \Log::warning('API login failed: user not found', [
                'email' => $request->email,
                'login_by' => $request->login_by,
                'user_type' => $request->user_type,
                'ip' => $request->ip(),
            ]);
            return $this->apiErrorResponse(translate('User not found'), 401);
        }
    }
    ...
    {
        $validator = Validator::make($request->all(), [
            'social_provider' => 'required|in:facebook,google,twitter,apple',
            'access_token' => 'required|string',
            'secret_token' => [
                Rule::when($request->social_provider === 'twitter', ['required', 'string'], ['nullable'])
            ]
        ]);

        if ($validator->fails()) {
            \Log::warning('API social login validation failed', [
        ...
        if ($social_user == null) {
            \Log::warning('API social login failed: provider mismatch', [
                'social_provider' => $request->social_provider,
                'ip' => $request->ip(),
            ]);
            return $this->apiErrorResponse(translate('No social provider matches'), 422);
        }
        ...
        if ($social_user_details == null) {
            \Log::warning('API social login failed: no social account match', [
                'social_provider' => $request->social_provider,
                'ip' => $request->ip(),
            ]);
            return $this->apiErrorResponse(translate('No social account matches'), 422);
        }

        $providerId = (string) $social_user_details->id;
        ...
        if ($existingUserByProviderId) {
            ...
            $existing_or_new_user->provider_id = $providerId;
    ...
    public function guestUserAccountCreate(Request $request)
    {
        ...
        if($success == 0){
            \Log::error('API guest account create failed during email dispatch', [
                'email' => $request->email,
                'ip' => $request->ip(),
            ]);
            return $this->apiErrorResponse(translate('Something went wrong!'), 500);
        }
