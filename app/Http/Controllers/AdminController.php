...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        $data['total_store_products'] = Product::where('approved', 1)->where('published', 1)->where('added_by', 'admin')->count();
        ...
        $data['total_store_sale'] = Order::where("seller_id", $admin_id)->sum('grand_total');
        $data['payment_type_wise_store_sale'] = Order::select(DB::raw('case
            ...
            ->groupBy(DB::raw('1'))
            ->get();
        $data['store_product_rating'] = Product::where('added_by', 'admin')->where('rating', '!=', 0)->avg('rating');
        $data['total_store_order'] = Order::where("seller_id", $admin_id)->count();
    ...
    public function store_top_categories(Request $request)
    {
        $storeTopCategoryQuery = Order::query();
        $storeTopCategoryQuery->select('categories.id', 'categories.name', 'categories.cover_image', DB::raw('SUM(order_details.price + order_details.tax) as total'))
        ...
        if ($request->interval_type != 'all') {
            $storeTopCategoryQuery->where('orders.created_at', '>=', DB::raw('DATE_SUB(NOW(), INTERVAL 1 ' . $request->interval_type . ')'));
        }
        $store_top_categories = $storeTopCategoryQuery->groupBy('categories.name')
            ->orderBy('total', 'desc')
            ->limit(5)
            ->get();

        return view('backend.dashboard.store_top_categories', compact('store_top_categories'))->render();
    }

    public function store_top_brands(Request $request)
    {
        $storeTopBrandQuery = Order::query();
        $storeTopBrandQuery->select('brands.id', 'brands.name', 'brands.logo', DB::raw('SUM(order_details.price + order_details.tax) as total'))
            ->leftJoin('order_details', 'orders.id', '=', 'order_details.order_id')
            ->leftJoin('products', 'order_details.product_id', '=', 'products.id')
            ->leftJoin('brands', 'products.brand_id', '=', 'brands.id')
            ->where('orders.delivery_status', '=', 'delivered')
            ...
            ->whereRaw('products.added_by = "admin"');
        if ($request->interval_type != 'all') {
            $storeTopBrandQuery->where('orders.created_at', '>=', DB::raw('DATE_SUB(NOW(), INTERVAL 1 ' . $request->interval_type . ')'));
        }
        $store_top_brands = $storeTopBrandQuery->groupBy('brands.name')
            ->orderBy('total', 'desc')
            ->limit(5)
            ->get();

        return view('backend.dashboard.store_top_brands', compact('store_top_brands'))->render();
    }
