...
class CustomLabelController extends Controller
{
    ...
    {
        $sort_search = null;
        $customLabelUserType = $request->filled('custom_label_user_type') ? $request->custom_label_user_type : 'all';
        if (in_array($customLabelUserType, ['in_house', 'inhouse'], true)) {
            $customLabelUserType = 'store';
        }
        $custom_labels = CustomLabel::whereHas('user');

        if ($customLabelUserType != 'all') {
            $adminId = get_admin()->id;
            $custom_labels = $customLabelUserType === 'store'
