...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        $storeResponse = (new OrderController)->store($request);
        if ($storeResponse instanceof \Illuminate\Http\RedirectResponse) {
            return $storeResponse;
        }

        if ($request->session()->get('combined_order_id') == null) {
            flash(translate('Order could not be processed. Please try again.'))->warning();
            return redirect()->route('checkout');
        }
        ...
        if ($request->session()->get('combined_order_id') != null) {
            ...
            if (class_exists($decorator)) {
                ...
                $combined_order = CombinedOrder::find($request->session()->get('combined_order_id'));
                if (!$combined_order) {
                    Log::warning('Checkout manual payment failed: combined order missing', [
                        'combined_order_id' => $request->session()->get('combined_order_id'),
                        'user_id' => auth()->id(),
                        'payment_option' => $request->payment_option,
                    ]);
                    flash(translate('Order data is missing. Please try again.'))->warning();
                    return redirect()->route('checkout');
                }
    ...
    {
        if (!isset($guest_shipping_info['email']) || empty($guest_shipping_info['email'])) {
            $guest_shipping_info['email'] = $guest_shipping_info['phone'] . '@guest.com';
        }
        ...
        $validator = Validator::make($guest_shipping_info, [
            ...
            'state_id' => get_setting('has_state') == 1 ? 'nullable|integer' : 'nullable|integer',
        ...
        // Ensure country code is handled if missing
        $country_code = $guest_shipping_info['country_code'] ?? '88';
        $user->phone = '+'.$country_code.$guest_shipping_info['phone'];
        $user->password = Hash::make($password);
        $user->email_verified_at = date('Y-m-d H:m:s'); // Auto-verify guest accounts
        $user->save();
    ...
    public function checkout_done1($combined_order_id, $payment)
    {
        ...
        if (!$combined_order) {
            Log::warning('Checkout done1 skipped: combined order missing', [
        ...
        foreach ($combined_order->orders as $key => $order) {
            ...
            if (!$order) {
                Log::warning('Checkout done1 skipped missing order row', [
                    'combined_order_id' => $combined_order_id,
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        $combined_order = CombinedOrder::find($combined_order_id);
        if (!$combined_order) {
            Log::warning('Checkout done failed: combined order missing', [
                'combined_order_id' => $combined_order_id,
                'payment' => $payment,
                'user_id' => auth()->id(),
            ]);
            flash(translate('Order data is missing.'))->warning();
            return redirect()->route('home');
        }
        ...
        foreach ($combined_order->orders as $key => $order) {
            $order = Order::find($order->id);
            if (!$order) {
                Log::warning('Checkout done skipped missing order row', [
                    'combined_order_id' => $combined_order_id,
                    'order_id' => $order->id ?? null,
                ]);
                continue;
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if (get_setting('shipping_type') == 'carrier_wise_shipping') {
            ...
            $zone = Country::where('id', $country_id)->first()?->zone_id ?? 0;
    ...
    public function order_confirmed()
    {
        ...
        if (!$combined_order_id) {
            flash(translate('No order found to confirm.'))->warning();
            return redirect()->route('home');
        }

        $combined_order = CombinedOrder::find($combined_order_id);
        if (!$combined_order) {
            flash(translate('Order confirmation data is missing.'))->warning();
            return redirect()->route('home');
        }
    ...
    public function updateDeliveryAddress(Request $request)
    {
        ...
        $selectedAddress = $user != null ? Address::find($request->address_id) : null;
        if ($user != null && !$selectedAddress) {
            Log::warning('updateDeliveryAddress failed: address not found', [
    ...
    public function updateDeliveryInfo(Request $request)
    {
        ...
        $deliveryAddress = $user != null && isset($carts[0]) ? Address::find($carts[0]->address_id) : null;
        if ($user != null && !$deliveryAddress) {
            Log::warning('updateDeliveryInfo failed: cart address missing', [
                'user_id' => auth()->id(),
                'cart_address_id' => $carts[0]->address_id ?? null,
            ]);
            return view('frontend.partials.cart.cart_summary', compact('carts', 'proceed'))->render();
        }
        $country_id = $user != null ? $deliveryAddress->country_id : $request->country_id;
        $city_id = $user != null ? $deliveryAddress->city_id : $request->city_id;
        $area_id = $user != null ? $deliveryAddress->area_id : $request->area_id;
        $shipping_info['country_id'] = $country_id;
        $shipping_info['city_id'] = $city_id;
        $shipping_info['area_id'] = $area_id;   
