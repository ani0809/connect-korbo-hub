...
class HomeController extends Controller
{
    ...
    {
        try {
            $missingCoreTables = !\Illuminate\Support\Facades\Schema::hasTable('business_settings')
                || !\Illuminate\Support\Facades\Schema::hasTable('shops')
                || !\Illuminate\Support\Facades\Schema::hasTable('categories');

            if ($missingCoreTables) {
                return response()->view('system.db_not_ready', [], 503);
            }
        } catch (\Throwable $e) {
            return response()->view('system.db_not_ready', [], 503);
        }
        ...
        $homepage = get_setting('homepage_select');
        if ($homepage === 'thecore') {
            $layoutItems = get_thecore_homepage_layout();
            return view('frontend.thecore.index', compact('featured_categories', 'hot_categories', 'lang', 'layoutItems'));
        }
    ...
    public function load_newest_product_section(Request $request)
    {
        ...
        if ($request->has('page') && is_numeric($request->page)) {
            ...
            $newest_products = filter_products(Product::latest())
                ...
                ->take($limit)
                ->get();
            return view('frontend.partials.newest_product_cards', compact('newest_products'));
        }
        ...
        $section_id = $request->input('section_id', 'default');
        $custom_title = $request->input('custom_title', '');
        return view('frontend.' . get_setting('homepage_select') . '.partials.newest_products_section', compact('newest_products', 'section_id', 'custom_title'));
    }

    public function load_featured_section(Request $request)
    {
        $section_id = $request->input('section_id', 'section_featured');
        return view('frontend.' . get_setting('homepage_select') . '.partials.featured_products_section', compact('section_id'));
    }
