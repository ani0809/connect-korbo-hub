...
class NoteController extends Controller
{
    ...
    {
        $sort_search = null;
        $noteUserType = $request->filled('note_user_type') ? $request->note_user_type : 'all';
        if (in_array($noteUserType, ['in_house', 'inhouse'], true)) {
            $noteUserType = 'store';
        }
        ...
        if ($noteUserType != 'all') {
            ...
            $notes = $noteUserType === 'store'
                ? $notes->where('user_id', $adminId)
                : $notes->where('user_id', '!=', $adminId);
        }
