...
class CibatoUploadController extends Controller
{
    ...
    {
        return view('uploader.cibato-uploader');
    }
    ...
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile('cibato_file')) {
            $upload = new Upload;
            $extension = strtolower($request->file('cibato_file')->getClientOriginalExtension());
            ...
            if (isset($type[$extension])) {
                ...
                $arr = explode('.', $request->file('cibato_file')->getClientOriginalName());
                ...
                if ($extension == 'webp') {
                    ...
                    // Load the dirty svg
                    $dirtySVG = file_get_contents($request->file('cibato_file'));

                    // Pass it to the sanitizer and get it back clean
                    $cleanSVG = $sanitizer->sanitize($dirtySVG);

                    // Load the clean svg
                    file_put_contents($request->file('cibato_file'), $cleanSVG);
                }

                $size = $request->file('cibato_file')->getSize();
                ...
                if ($type[$extension] == 'image' && $extension != 'svg') {
                    ...
                    try {
                        ...
                        $img = Image::make($request->file('cibato_file')->getRealPath())->encode($extension, 75);
                        $height = $img->height();
                        $width = $img->width();
                        ...
                        // Fallback: keep original uploaded image in public/uploads/all
                        $extension = strtolower($request->file('cibato_file')->getClientOriginalExtension());
                        $filename = Str::random(40) . '.' . $extension;
                        ...
                        $request->file('cibato_file')->move($targetDir, $filename);
                        clearstatcache();
                        $savedPath = base_path('public/') . $path;
                        $size = file_exists($savedPath) ? filesize($savedPath) : $request->file('cibato_file')->getSize();
                    }
                } else {
                    $path = $request->file('cibato_file')->store('uploads/all', 'local');
                }

                // Final guard: never save DB row if the image file is not physically present.
                $publicFilePath = base_path('public/') . $path;
                if (!file_exists($publicFilePath) && $request->hasFile('cibato_file')) {
                    $targetDir = dirname($publicFilePath);
                    if (!file_exists($targetDir)) {
                        @mkdir($targetDir, 0777, true);
                    }
                    try {
                        Image::make($request->file('cibato_file')->getRealPath())->save($publicFilePath);
                    } catch (\Throwable $th) {
                        @copy($request->file('cibato_file')->getRealPath(), $publicFilePath);
                    }
                    clearstatcache();
                }

                if (!file_exists($publicFilePath)) {
                    \Log::error('Uploader file write failed before DB save', [
                        'path' => $path,
                        'user_id' => Auth::id(),
                        'extension' => $extension,
                    ]);
                    return response()->json(['status' => 'error', 'message' => 'File write failed'], 422);
                }
                ...
                if ($storageDriver != 'local') {
                    ...
                    $uploaded = Storage::disk($storageDriver)->put(
                        $path,
                    ...
                    // Keep local fallback when remote upload fails.
                    if ($uploaded && $arr[0] != 'updates') {
    ...
    public function destroy($id)
    {
        ...
        try {
            $storageDriver = config('filesystems.default');
            if ($storageDriver != 'local') {
                Storage::disk($storageDriver)->delete($upload->file_name);
    ...
    public function get_preview_files(Request $request)
    {
        ...
        foreach ($files as $file) {
            $file['file_name'] = uploaded_asset($file->id);
