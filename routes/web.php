...
Route::controller(CibatoUploadController::class)->group(function () {
    Route::post('/cibato-uploader', 'show_uploader');
    Route::post('/cibato-uploader/upload', 'upload');
    Route::get('/cibato-uploader/get-uploaded-files', 'get_uploaded_files');
    Route::post('/cibato-uploader/get_file_by_ids', 'get_preview_files');
    Route::get('/cibato-uploader/download/{id}', 'attachment_download')->name('download_attachment');
});
...
Route::controller(HomeController::class)->group(function () {
    ...
    Route::permanentRedirect('/inhouse', '/store-products');
    Route::get('/store-products', 'inhouse_products')->name('store_products.all');
...
if (get_setting('wallet_system') == 0) {
    Route::get('/wallet', function () {
        abort(404);
    })->name('wallet.index');

    Route::post('/recharge', function () {
        abort(404);
...
if (get_setting('conversation_system') == 0) {
    Route::any('/conversations/{any?}', function () {
        abort(404);
    })->where('any', '.*');
}

if (get_setting('product_query_activation') == 0) {
    Route::any('/product-queries/{any?}', function () {
        abort(404);
    })->where('any', '.*');
}

if (get_setting('blog_system') == 0) {
    ...
    Route::post('/blog/generate-slug', function () {
        abort(404);
    })->name('generate.slug');
