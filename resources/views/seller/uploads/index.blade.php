    ...
    <form id="sort_uploads" action="">
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-3 ml-auto mr-0">
                <select class="form-control form-control-xs cibato-selectpicker" name="sort" onchange="sort_uploads()">
		...
		<div class="card-body">
						...
						<div class="cibato-file-box">
							<div class="dropdown-file" >
								...
								<div class="dropdown-menu dropdown-menu-right">
									...
									@php
										$file_path = uploaded_asset($file->id);
									@endphp
									<a href="{{ $file_path }}" target="_blank" download="{{ $file_name }}.{{ $file->extension }}" class="dropdown-item">
										<i class="la la-download mr-2"></i>
										<span>{{ translate('Download') }}</span>
									</a>
									<a href="javascript:void(0)" class="dropdown-item" onclick="copyUrl(this)" data-url="{{ $file_path }}">
							...
							<div class="card card-file cibato-uploader-select c-default" title="{{ $file_name }}.{{ $file->extension }}">
			...
			<div class="cibato-pagination mt-3">
...
@section('script')
	<script type="text/javascript">
		function detailsInfo(e){
			...
			$.post('{{ route('seller.my_uploads.info') }}', {_token: CIBATO.data.csrf, id:id}, function(data){
                $('#info-modal-content').html(data);
				// console.log(data);
			}).fail(function(){
                $('#info-modal-content').html('<div class="text-danger text-center py-4">{{ translate("Failed to load file information.") }}</div>');
            });
		}
		function copyUrl(e) {
			var url = $(e).data('url');
		    ...
		    try {
			    ...
			    CIBATO.plugins.notify('success', '{{ translate('Link copied to clipboard') }}');
			} catch (err) {
			    CIBATO.plugins.notify('danger', '{{ translate('Oops, unable to copy') }}');
			}
		    $temp.remove();
		}
