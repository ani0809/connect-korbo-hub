                            ...
                            @if ($product->choice_options != null)
                                @foreach (json_decode($product->choice_options) as $key => $choice)
                                    <div class="row no-gutters border-bottom-dashed border-soft-light py-2">
                                        ...
                                        <div class="col-12">
                                            <div class="cibato-radio-inline">
                                                @foreach ($choice->values as $key => $value)
                                                    <label class="cibato-megabox pl-0 mr-2 my-1">
                                                        <input type="radio"
                                                            name="attribute_id_{{ $choice->attribute_id }}"
                                                            value="{{ $value }}"
                                                            @if ($key == 0) checked @endif>
                                                        <span
                                                            class="cibato-megabox-elem rounded-0 d-flex align-items-center justify-content-center py-1 px-3 rounded-1">
                                                            {{ $value }}
                                                        </span>
                                                    </label>
                                                @endforeach
                                    ...
                                    <div class="col-12">
                                        <div class="cibato-radio-inline">
                                            @foreach (json_decode($product->colors) as $key => $color)
                                                <label class="cibato-megabox pl-0 mr-2 mb-0" data-toggle="tooltip"
                                                    data-title="{{ get_single_color_name($color) }}">
                                                    <input type="radio" name="color"
                                                        value="{{ get_single_color_name($color) }}"
                                                        @if ($key == 0) checked @endif>
                                                    <span
                                                        class="cibato-megabox-elem rounded-0 d-flex align-items-center justify-content-center p-1 rounded-1">
            ...
            <div class="pb-10px">
                ...
                <div class="mt-2">
                    ...
                    <div class="d-flex align-items-center flex-wrap">
                        ...
                        <div class="d-flex align-items-center cibato-plus-minus border border  border-soft-light rounded-1">
                            <!--Decrement-->
...
<div class="w-100 px-30px position-absolute bottom-0 bg-white right-offcavas-footer product-details-page pt-20px pb-20px border-top border-soft-light" style="box-shadow: none!important;">
    <div class="d-flex flex-wrap flex-md-nowrap align-items-center mb-2">
        <button id="added_to_cart_btn" type="button" @if (Auth::check() || get_Setting('guest_checkout_activation') == 1) onclick="addToCart()" @else onclick="showLoginModal()" @endif class="text-blue border-0 rounded-1 fs-14 fw-bold bg-soft-blue hov-bg-blue hov-text-white py-15px px-20px w-100 mb-2 mb-md-0 mr-0 mr-md-2 add-to-cart">{{ translate('Add to cart') }} <span id="add_to_cart_count"></span></button>
        <button type="button" @if (Auth::check() || get_Setting('guest_checkout_activation')==1) onclick="buyNow()" @else onclick="showLoginModal()" @endif class="border-0 rounded-1 fs-14 fw-bold py-15px px-20px w-100 buy-now">{{ translate('Buy Now') }}</button>
    </div>
    @if (get_setting('whatsapp_order') == 1)
        @php
            $waCanvasStoreName = env('APP_NAME');
            $waCanvasProductTitle = $product->getTranslation('name');
            $waCanvasProductUrl = URL::to('/product') . '/' . $product->slug;
            $waCanvasTemplate = get_setting('order_messege_template');
            $waCanvasMessage = str_replace(
                ['[[storeName]]', '[[productTitle]]', '[[productUrl]]'],
                [$waCanvasStoreName, $waCanvasProductTitle, $waCanvasProductUrl],
                $waCanvasTemplate
            );
            $waCanvasNumber = preg_replace('/[^0-9]/', '', env('WHATSAPP_NUMBER'));
            $waCanvasOrderUrl = "https://wa.me/{$waCanvasNumber}?text=" . urlencode($waCanvasMessage);
        @endphp
        @if (($product->added_by == 'seller' && get_setting('whatsapp_order_seller_prods') == 1) || ($product->added_by == 'admin'))
            <a href="{{ $waCanvasOrderUrl }}" target="_blank" rel="noopener noreferrer"
                class="btn-order-whatsapp-full d-flex align-items-center justify-content-center w-100 border-0 rounded-1 fs-14 fw-bold text-white py-15px px-20px mt-2 mb-2 text-decoration-none has-transition">
                <i class="lab la-whatsapp fs-22 mr-2"></i>
                <span>{{ translate('Order on WhatsApp') }}</span>
            </a>
        @endif
    @endif
    <div class="">
