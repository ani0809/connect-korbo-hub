            ...
            <div class="container d-flex flex-column">
                ...
                <section id="description">
                    <div class="py-30px px-30px border  bg-white border-light-gray rounded-2">
                        <div class="mw-100 overflow-hidden text-left cibato-editor-data">
                ...
                <div class="modal-body c-scrollbar-light">
                    ...
                    <div class="my-4 my-lg-5">
                     ...
                     <div class="pb-3">
                        ...
                        <div class="cibato-share text-center"></div>
                     </div>
                </div>
            </div>
        </div>
...
@section('script')
   ...
   <script type="text/javascript">
        ...
        function SKUCopyToClipboard() {
            ...
            if (skuText === "") {
                CIBATO.plugins.notify('warning', 'No SKU to copy');
                return;
            }
            ...
            try {
                document.execCommand("copy");
                CIBATO.plugins.notify('success', '{{ translate('SKU copied to clipboard') }}');
            } catch (err) {
                CIBATO.plugins.notify('danger', '{{ translate('Oops, unable to copy') }}');
            }
    ...
    //     });
            ...
            @if (isCustomer())
                @if ($review_status == 1)
                    $.post('{{ route('product_review_modal') }}', {
                        ...
                        CIBATO.extra.inputRating();
                    });
                @else
                    CIBATO.plugins.notify('warning', '{{ translate("Sorry, You need to buy this product to give review.") }}');
                @endif
            @elseif (Auth::check() && !isCustomer())
                CIBATO.plugins.notify('warning', '{{ translate("Sorry, Only customers can give review.") }}');
            @else
                $('#login_modal').modal('show');
            @endif
        }
        ...
        function showSizeChartDetail(id, name){
            ...
            if (id == 0) {
                CIBATO.plugins.notify('warning', '{{ translate("Sorry, There is no size guide found for this product.") }}');
                return false;
            }
            $.ajax({
                type: "GET",
