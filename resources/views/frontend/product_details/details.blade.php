                ...
                <div class="d-flex flex-wrap align-items-center">
                    ...
                    <div class="">
                        @if ($detailedProduct->auction_end_date > strtotime('now'))
                            <div class="cibato-count-down align-items-center" data-date="{{ date('Y/m/d H:i:s', $detailedProduct->auction_end_date) }}"></div>
                        @else
                                ...
                                <div class="py-10px {{ $key == 0 ? '' : 'variant-item' }} {{ $key <9 ? '' : 'variant-item-none' }}">
                                    ...
                                    <div class="variant-wrapper">
                                        @foreach ($choice->values as $key => $value)
                                            <label class="rounded-1 bg-white cursor-pointer cibato-megabox mb-1">
                                                <input type="radio" name="attribute_id_{{ $choice->attribute_id }}" value="{{ $value }}" @if ($key == 0) checked @endif>
                                                <div class="variant-item-select cibato-megabox-elem px-10px">
                            ...
                            <div class="py-10px  {{ $product_variations>1 ? 'variant-item':''}}">
                                ...
                                <div class="variant-wrapper">
                                    @foreach (json_decode($detailedProduct->colors) as $key => $color)
                                        <label class="cibato-megabox rounded-1 bg-white cursor-pointer" data-title="{{ get_single_color_name($color) }}">
                                            <input type="radio" name="color" value="{{ get_single_color_name($color) }}" @if ($key == 0) checked @endif>
                                            <div class="d-flex align-items-center variant-item-select cibato-megabox-elem px-15px">
                ...
                <div class="border-dashed border-1 border-soft-light rounded-2 overflow-hidden mt-4 mb-1 px-20px pt-15px pb-20px">
                    ...
                    <div class="d-flex flex-wrap flex-md-nowrap align-items-center">
                        <button type="button" id="added_to_cart_btn" @if (Auth::check() || get_Setting('guest_checkout_activation')==1) onclick="addToCart()" @else onclick="showLoginModal()" @endif
                            class="text-primary border-0 rounded-1 fs-14 fw-bold bg-soft-primary hov-bg-primary hov-text-white py-20px px-20px w-100 mb-2 mb-md-0 mr-0 mr-md-2 add-to-cart">{{translate('Add to Cart')}} <span id="add_to_cart_count">(01)</span>
                        </button>
                        <button type="button" @if (Auth::check() || get_Setting('guest_checkout_activation')==1) onclick="buyNow()" @else onclick="showLoginModal()" @endif
                            class="border-0 rounded-1 fs-14 fw-bold py-20px px-20px w-100 buy-now">{{ translate('Buy Now') }}
                        </button>
                    </div>
                    @if (get_setting('whatsapp_order') == 1)
                        @php
                            $waStoreName = env('APP_NAME');
                            $waProductTitle = $detailedProduct->getTranslation('name');
                            $waProductUrl = URL::to('/product') . '/' . $detailedProduct->slug;
                            $waTemplate = get_setting('order_messege_template');
                            $waMessage = str_replace(
                                ['[[storeName]]', '[[productTitle]]', '[[productUrl]]'],
                                [$waStoreName, $waProductTitle, $waProductUrl],
                                $waTemplate
                            );
                            $waNumber = preg_replace('/[^0-9]/', '', env('WHATSAPP_NUMBER'));
                            $waOrderUrl = "https://wa.me/{$waNumber}?text=" . urlencode($waMessage);
                        @endphp
                        @if (($detailedProduct->added_by == 'seller' && get_setting('whatsapp_order_seller_prods') == 1) || ($detailedProduct->added_by == 'admin'))
                            <a href="{{ $waOrderUrl }}" target="_blank" rel="noopener noreferrer"
                                class="btn-order-whatsapp-full d-flex align-items-center justify-content-center w-100 border-0 rounded-1 fs-14 fw-bold text-white py-20px px-20px mt-2 text-decoration-none has-transition">
