        ...
        <div class="container">
            <form class="form-default" data-toggle="validator" action="{{ route('payment.checkout') }}" role="form" method="POST" id="checkout-form">
                ...
                <div class="row cols-xs-space cols-sm-space cols-md-space">
                    <div class="col-lg-8">
                        <div>
                            <div class="card rounded-0 mb-3 border shadow-none">
                                <div class="card-header border-bottom-0 py-3 py-xl-4" id="headingShippingInfo">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="ml-2 fs-19 fw-700">{{ translate('Billing Details') }}</span>
                                    </div>
                                </div>
                                <div id="collapseShippingInfo" class="show" aria-labelledby="headingShippingInfo">
                                    <div class="card-body" id="shipping_info">
                                        @include('frontend.partials.cart.shipping_info', ['carts' => $carts, 'address_id' => $address_id, 'shipping_info' => $shipping_info])
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Payment Info -->
                            <div id="checkout-payment-card" class="card rounded-0 mb-0 border shadow-none">
                                ...
                                <div id="collapsePaymentInfo" class="show" aria-labelledby="headingPaymentInfo">
                    ...
                    <!-- Sidebar -->
                    <div id="checkout-sidebar-column" class="col-lg-4 mt-4 mt-lg-0">
                        <!-- Delivery Info -->
                        <div class="card rounded-0 border shadow-none" style="margin-bottom: 2rem; overflow: visible !important;">
                            <div class="card-header border-bottom-0 py-3 py-xl-4 bg-white" id="headingDeliveryInfo">
                                <div class="d-flex align-items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                        <path id="Path_42357" data-name="Path 42357" d="M58,48A10,10,0,1,0,68,58,10,10,0,0,0,58,48ZM56.457,61.543a.663.663,0,0,1-.423.212.693.693,0,0,1-.428-.216l-2.692-2.692.856-.856,2.269,2.269,6-6.043.841.87Z" transform="translate(-48 -48)" fill="#9d9da6"/>
                                    </svg>
                                    <span class="ml-2 fs-19 fw-700">{{ translate('Your Order') }}</span>
                                </div>
                            </div>
                            <div id="collapseDeliveryInfo" class="show">
                                <div class="card-body" id="delivery_info">
                                    ...
                                    <div id="cart_summary" class="mt-2 mt-lg-1">
                                        @include('frontend.partials.cart.cart_summary', ['proceed' => 0, 'carts' => $carts, 'embedded' => true])
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script type="text/javascript">
        (function() {
            function repositionCheckoutPaymentCard() {
                var paymentCard = document.getElementById('checkout-payment-card');
                var sidebarCol = document.getElementById('checkout-sidebar-column');
                var shippingCard = document.getElementById('collapseShippingInfo');
                if (!paymentCard || !sidebarCol || !shippingCard) return;

                if (window.matchMedia('(max-width: 991.98px)').matches) {
                    if (sidebarCol.nextElementSibling !== paymentCard) {
                        sidebarCol.insertAdjacentElement('afterend', paymentCard);
                    }
                } else {
                    var shippingCardWrapper = shippingCard.closest('.card');
                    if (shippingCardWrapper && shippingCardWrapper.nextElementSibling !== paymentCard) {
                        shippingCardWrapper.insertAdjacentElement('afterend', paymentCard);
                    }
                }
            }

            document.addEventListener('DOMContentLoaded', repositionCheckoutPaymentCard);
            window.addEventListener('resize', repositionCheckoutPaymentCard);
        })();
       ...
       var carrierCount=0;
            ...
            if ($('#agree_checkbox').is(":checked")) {
                ...
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    ...
                    if(isOkShipping && isOkDelivery && isOkPayment) {
                        ...
                        CIBATO.plugins.notify('danger', '{{ translate("Please fill in all mandatory fields!") }}');
                ...
                CIBATO.plugins.notify('danger', '{{ translate('You need to agree with our policies') }}');
                deactivateButtonLoader(el);
            }
        }
            ...
            if ($('#agree_checkbox').is(":checked")) {
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    ...
                    if (offline_payment_active == '1' && $('.offline_payment_option').is(":checked") && $('#trx_id')
                        .val() == '') {
                        CIBATO.plugins.notify('danger', '{{ translate('You need to put Transaction id') }}');
                        deactivateButtonLoader(el);
                        return;
                    } else {
                        var allIsOk = false;
            ...
            function openOtpModal() {
                ...
                $.post("{{ route('checkout.send_purchase_otp') }}", {
                    ...
                    if(response.status){
                        ...
                        CIBATO.plugins.notify('danger', response.message);
                    }
        ...
        $(document).on("click", "#coupon-apply", function() {
            @if (Auth::check())
                @if(Auth::user()->user_type != 'customer')
                    CIBATO.plugins.notify('warning', "{{ translate('Please Login as a customer to apply coupon code.') }}");
                    return false;
                @endif
                ...
                $.ajax({
                    ...
                    success: function(data, textStatus, jqXHR) {
                        CIBATO.plugins.notify(data.response_message.response, data.response_message.message);
        ...
        function updateDeliveryAddress(id, city_id = 0, area_id=0) {
            $('.cibato-refresh').addClass('active');
            $.post('{{ route('checkout.updateDeliveryAddress') }}', {
                ...
                $('.cibato-refresh').removeClass('active');
    ...
    <script>
        $(document).ready(function() {
            @if(Auth::check())
                ...
                @endif
            @else
                // For Guest - Auto load Bangladesh cities and update delivery info
                get_city_by_country(18);
                @if(get_setting('shipping_type') == 'carrier_wise_shipping')
                    updateDeliveryAddress(18);
                @endif

                // Listen for city changes to update delivery address/shipping fee
                $(document).on('change', '[name="city_id"]', function() {
                    var city_id = $(this).val();
                    if (city_id) {
                        updateCityShipping(city_id);
                    }
                });
                ...
                $(document).on('change', '[name="area_id"]', function() {
                    ...
                    if (city_id) {
                        updateDeliveryAddress(18, city_id, area_id);
                    }
                });
            @endif
        });

        function updateCityShipping(city_id) {
            updateDeliveryAddress(18, city_id);
        }

        function removeFromCartView(e, key) {
            CIBATO.extra.removeFromCart(key);
        }
        ...
        function updateQuantity(key, element) {
            $.post('{{ route('cart.updateQuantity') }}', {
                ...
                if (data.status == 0) {
                    CIBATO.plugins.notify('danger', data.message);
                } else {
                    updateNavCart(data.nav_cart_view, data.cart_count);
                    // Refresh checkout sections
                    var city_id = $('[name="city_id"]').val() || 0;
                    updateDeliveryAddress(18, city_id);
                }
            });
        }

        // Override standard AJZ removeFromCart success callback for checkout page
        CIBATO.extra.removeFromCart = function(key) {
