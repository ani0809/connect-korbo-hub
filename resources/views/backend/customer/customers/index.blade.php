    ...
    <script type="text/javascript">
        ...
        function bulk_delete() 
        {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', 'Selected customers Deleted successfully');
                        hideBulkActionModal();
                        getCustomers(currentTab);
                    }
                },
        ...
        function bulkDeleted() 
        {
            if ($('.check-one:checked').length == 0) {
                CIBATO.plugins.notify('danger', '{{ translate('Please select at least one customer') }}');
                return;
            }
        ...
        $(document).on('click', '#confirm-recharge', function () {
            ...
            if (!amount) {
                CIBATO.plugins.notify('warning', 'Please fill amount');
                return;
            }

            if (!trx_id) {
                CIBATO.plugins.notify('warning', 'Please fill transaction id');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    CIBATO.plugins.notify('success', 'Wallet recharged successfully');
                    closeRightcanvas();
                    location.reload();
                },
                error: function () {
