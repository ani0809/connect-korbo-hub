			...
			<div class="card">
				...
				<div class="card-body">
					<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
						...
						<!-- Show Compare Icon -->
						<div class="form-group row">
							<label class="col-md-3 col-from-label">{{translate('Show Compare Icon?')}}</label>
							<div class="col-md-8">
								<label class="cibato-switch cibato-switch-success mb-0">
									<input type="hidden" name="types[]" value="show_compare_icon">
									<input type="checkbox" name="show_compare_icon"
										@if(get_setting('show_compare_icon') == 'on') checked @endif>
									<span></span>
								</label>
							</div>
						</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-8">
									<div class="input-group">
										...
										<input type="text" class="form-control cibato-color-input" placeholder="#000000"
											name="{{ $style->name }}" value="{{ get_setting($style->name) }}">
										<div class="input-group-append">
											<span class="input-group-text p-0">
												<input data-target="{{ $style->name }}"
													class="cibato-color-picker border-0 size-40px" type="color"
...
@section('script')
	...
	<script>
		$(document).ready(function () {
			...
			function updateUI() {
				...
				toggleVisibility('show_compare_icon', 'compare-visibility');
				toggleVisibility('header_stikcy', 'stikcy-header-visibility', 'sticky-top');
			}

			updateUI();

			$('input[name="show_language_switcher"], input[name="show_currency_switcher"], input[name="show_compare_icon"], input[name="header_stikcy"]').on('change', function () {
				updateUI();
			});
		});
	</script>
	...
	<script>
		$(document).ready(function () {
			...
			$('.cibato-color-picker').on('input change', function (e) {
				e.preventDefault();
	...
	<script>
		$(document).ready(function () {
			...
			$('.cibato-color-picker').on('input change', function () {
				const color = $(this).val();
				const target = $(this).data('target');
				$('input[name="' + target + '"]').val(color).trigger('change');
			});
				...
				if (fileId && fileId !== previousFileId) {
					...
					$.ajax({
						...
						success: function (res) {
							if (res.success) {
								// Use backend-provided resolved URL to avoid path mismatches.
								let imagePath = res.file_url || ('{{ asset('') }}' + (res.file_name || ''));
