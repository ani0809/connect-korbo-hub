         ...
         <div
            ...
            <div class="table-tabs-container flex-grow-1">
                <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                    @foreach ($seller_types as $seller_type_tab)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}"
                                ...
                                id="{{ Str::slug($seller_type_tab) }}-tab"
                                onclick="changeTab(this, '{{ Str::slug($seller_type_tab) }}')" role="tab"
                                aria-controls="{{ Str::slug($seller_type_tab) }}">
                                {{ translate($seller_type_tab) }}
                            </button>
                        </li>
                    @endforeach
                </ul>
        ...
        <div class="tab-filter-bar">
            <form class="" id="sort_orders" action="" method="GET">
                <div class="card-header row  border-0 pb-0 mt-2">
                    ...
                    @if(collect($seller_types ?? [])->contains('Seller'))
                    <div class="col-md-2 mr-0 px-0 inner-select ml-1">
                        <select class="form-control  cibato-selectpicker mb-2 mb-md-0 bg-light" id="user_id" name="user_id" onchange="sort_orders()">
                    ...
                    <div class="col-md-2 ml-auto pr-0 pr-md-1 pl-0 inner-select ">
                        <select class="form-control  cibato-selectpicker mb-2 mb-md-0 bg-light" name="payment_status"
                    ...
                    <div class="col-md-2 ml-auto pl-0 inner-select input-group mb-0 border border-light px-3 bg-light rounded-1">
                        <input type="text" class="cibato-date-range form-control form-control-sm border-0 px-2 bg-transparent" value=""
...
@section('script')
    <script type="text/javascript">
        ...
        function getOrders(slug, page = 1) {
            ...
            let dateRange = $('.cibato-date-range').val();
            let col = '{{ $col ?? '' }}';
            let status = '{{ $status ?? '' }}';
        ...
        function changeTab(button, statusSlug) {
            ...
            if(statusSlug === 'inhouse-products' || statusSlug === 'inhouse' || statusSlug === 'store'){
                seller_type = 'admin';
            }
            else if (statusSlug === 'seller-products' || statusSlug === 'seller'){
                seller_type = 'seller';
            }
            else{
        ...
        $(document).ready(function() {
            $('.cibato-date-range').on('apply.daterangepicker', function(ev, picker) {
                getOrders(currentTab);
            });
            $('.cibato-date-range').on('change', function() {
                getOrders(currentTab);
            });
        });
        ...
        function bulk_unpaid_order_payment_notification(){
            ...
            if(orderIds.length > 0){
                ...
                CIBATO.plugins.notify('danger', '{{ translate('Please Select Order first.') }}');
            }
        }
