...
<div class="cibato-titlebar text-left mt-2 mb-3">
    ...
    <div class="card popup-settings mt-3">
        <div class="card-body">
            <form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- Watermark Position -->
            <div class="form-group row gutters-5 mb-0 align-items-center">
                <label class="col-md-3 col-from-label fw-700">{{translate('Dynamic Popup Duration (seconds)')}}</label>
                <div class="col-md-7">
                    <input type="hidden" name="types[]" value="dynamic_popup_duration">
                    <input type="number" integer-only min="5" step="1" class="form-control" placeholder="{{translate('Duration in seconds')}}" name="dynamic_popup_duration" value="{{ get_setting('dynamic_popup_duration') }}">
                </div>
                     <!-- Update Button -->
            ...
            <table class="table cibato-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="cibato-switch cibato-switch-primary mb-0">
								<input
    ...
    <script type="text/javascript">
        ...
        function update_dynamic_popup_status(el){
            ...
            $.post('{{ route('dynamic-popups.update-status') }}', {_token:'{{ csrf_token() }}', id:id, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Dynamic popup status updated successfully') }}');
                }
            });
        }
