...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table cibato-table table-striped table-bordered" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th width="10%">#</th>
...
@section('script')
    ...
    function updateStatus(el) {
            ...
            $.post('{{ route('countries.status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Country status updated successfully') }}');
                }
                else{
