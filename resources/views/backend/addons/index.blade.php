...
@section('content')
    ...
    <div class="">
        <div class="row ">
            <div class="col-md-6">
                <div class="nav border-bottom cibato-nav-tabs">
			...
			{{-- <div class="col mt-3 mt-md-0 text-center text-md-right">
                <a href="https://licensing.cibato.com/addons/activate" class="btn btn-primary" target="_blank">
					{{ translate('Activate Addon Link') }}
				</a>
            </div> --}}
            <div class="col mt-3 mt-md-0 text-center text-md-right">
                <a href="https://licensing.cibato.com/addons/activate" class="btn btn-primary mb-3 mb-sm-0 mx-3 mx-md-0 mr-lg-3" target="_blank">
					{{ translate('Activate Addon Link') }}
				</a>
                                        ...
                                        <div class="align-items-center d-flex flex-column flex-md-row">
                                            ...
                                            @if (env('DEMO_MODE') != 'On')
                                                <div class="mr-md-3 ml-0 w-100 w-md-auto">
                                                    <p><small>{{ translate('License code')}}: </small>{{ $addon->purchase_code }}</p>
                                                </div>
                                            @endif
                                            <div class="ml-auto mr-0">
                                                <label class="cibato-switch mb-0">
   ...
   <script type="text/javascript">
    ...
    function updateStatus(el, id) {
        ...
        $.post('{{ route('addons.activation') }}', {_token:'{{ csrf_token() }}',id:id, status:status}, function(data){
            if (data == 1){
                CIBATO.plugins.notify('success', '{{ translate('Status updated successfully') }}');
            } else {
    ...
    $(document).on('click', '#back-btn, [data-dismiss="modal"]', function(){
        ...
        $(document).ready(function(){
            $.post('https://licensing.cibato.com/addons/public/addons', {item: 'ecommerce'}, function(data){
                //console.log(data);
                html = '';
                data.forEach((item, i) => {
                    if(item.link != null){
