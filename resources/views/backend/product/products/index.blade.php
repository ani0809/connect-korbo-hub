...
@section('content')
    ...
    <div class="cibato-titlebar text-left pb-5px">
            ...
            <form class="" id="sort_products" action="" method="GET">
                <div class="card-header row  border-0 pb-0 mt-2">
                    ...
                    <div class="col-md-2 mr-0 px-0 inner-select ml-1">
                        <select class="form-control  cibato-selectpicker mb-2 mb-md-0 bg-light" id="user_id" name="user_id" onchange="sort_products()">
                    ...
                    <div class="col-md-2 ml-auto pr-0 pr-md-3 pl-0 inner-select ">
                        <select class="form-control  cibato-selectpicker mb-2 mb-md-0 bg-light" name="type"
...
@section('script')
    <script type="text/javascript">
        ...
        function update_todays_deal(el){
            ...
            if('{{env('DEMO_MODE')}}' == 'On'){
                CIBATO.plugins.notify('info', '{{ translate('Data can not change in demo mode.') }}');
                return;
            }
            ...
            $.post('{{ route('products.todays_deal') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Todays Deal updated successfully') }}');
                }
                else{
        ...
        function update_published(el){
            ...
            $.post('{{ route('products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Published products updated successfully') }}');
                }
                else if(data == 3){
                    CIBATO.plugins.notify('danger', '{{ translate('GST verification is pending for this account.') }}');
                }
                else if(data == 4){
                    CIBATO.plugins.notify('warning', '{{ translate('Please assign GST details') }}');
                }
                else{
                    CIBATO.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function update_approved(el){
            ...
            $.post('{{ route('products.approved') }}', {
                ...
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Product approval update successfully') }}');
                }
                else{
                    CIBATO.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function update_featured(el){
            ...
            $.post('{{ route('products.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Featured products updated successfully') }}');
                }
                else{
                    CIBATO.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function single_delete(productId) {
            $.ajax({
                ...
                success: function(response) {
                    if (response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected item deleted successfully') }}');
                        hideBulkActionModal();
        ...
        function bulk_delete() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected items deleted successfully') }}');
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                }
        ...
        function bulk_publish() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected items Published successfully') }}');
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                },
                error: function () {
                    CIBATO.plugins.notify('danger', 'Something went wrong');
                }
            });
        }
        ...
        function bulk_feature() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success','{{ translate('Selected items added in featured successfully') }}' );
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                },
        ...
        function bulk_todays_deal() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected products have been marked as Today’s Deals.') }}');
                        hideBulkActionModal();
                        getProducts(currentTab);
                    }
                }
        ...
        function bulkDelete() {
            if ($('.check-one:checked').length == 0) {
                CIBATO.plugins.notify('danger', '{{ translate('Please select at least one item') }}');
                return;
            }
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    try {
                        ...
                        console.warn('initFooTable failed:', error);
                    }
        ...
        function changeTab(button, statusSlug) {
            ...
            if(statusSlug === 'store' || statusSlug === 'inhouse-products' || statusSlug === 'inhouse'){
                seller_type = 'admin';
            }
            else if (statusSlug === 'seller' || statusSlug === 'seller-products'){
                seller_type = 'seller';
            }
            else{
        ...
        function updateStocks(productId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Stock updated successfully') }}');
                        closeRightcanvas();
                    }
                },
                error: function() {
