    ...
    <script type="text/javascript">
        ...
        function update_featured(el){
            ...
            $.post('{{ route('categories.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Featured categories updated successfully') }}');
                }
                else{
        ...
        function update_hot(el){
            ...
            $.post('{{ route('categories.hot') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    CIBATO.plugins.notify('success', '{{ translate('Hot categories updated successfully') }}');
                }
                else{
                    CIBATO.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function bulk_feature() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected items added to featured categories successfully.') }}');
                        hideBulkActionModal(); 
                        getCategories(currentTab);
                    }
                },
        ...
        function bulk_hot() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        CIBATO.plugins.notify('success', '{{ translate('Selected items added to hot categories successfully.') }}');
                        hideBulkActionModal();
                        getCategories(currentTab);
                    }
                }
        ...
        function bulkFeatured() {
            if ($('.check-one:checked').length == 0) {
                CIBATO.plugins.notify('danger', '{{ translate('Please select at least one category') }}');
                return;
            }
            showBulkActionModal();
        ...
        function getCategories(slug, page = 1) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Always remove loader, even if response format differs.
                    var html = (response && response.html) ? response.html : response;
                    $('#tab-content').html(html);
                    try {
                        initFooTable();
                    } catch (e) {
                        // Keep content visible even if footable init fails.
                        console.warn('initFooTable failed', e);
                    }
                },
                error: function() {
