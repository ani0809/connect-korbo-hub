        ...
        <table class="table cibato-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg" class="text-center" width="10%">{{ translate('Products') }}</th>
                ...
                @foreach($categories as $key => $category)
                    ...
                    <tr>
                        ...
                        <td>
                         ...
                         <td>
                            <input type="text" class="form-control cibato-date-range rounded-2" id="date_range_{{ $category->id }}" value="{{ $start_date && $end_date ? $start_date . ' to ' . $end_date : '' }}" placeholder="{{translate('Select Date')}}" data-time-picker="true" data-format="DD-MM-Y HH:mm:ss" data-separator=" to " autocomplete="off">
                        </td>
                        <td class="text-right">
...
@section('script')
    ...
    $('#confirm-modal-switch').on('hidden.bs.modal', function () {
        ...
        function setDiscount(){
            ...
            if(discount < 0) {
                CIBATO.plugins.notify('danger', '{{ translate('Discount can not be less than 0') }}');
            }
            else{
                $.post('{{ route('set_product_discount') }}', {
                    ...
                    if(data == 1){
                        CIBATO.plugins.notify('success', '{{ translate('Category Wise Product Discount Set Successfully') }}');
                    }
                    location.reload();
                }).fail(function() {
